...
function forms() {
  ...
  function bindFormPost(form) {
    form.addEventListener('submit', e => {
      ...
      axios__WEBPACK_IMPORTED_MODULE_0__["default"].post('http://localhost:3000/requests', json).then(data => {
        console.log(data);
      }).catch(() => {
        console.log('error');
      }).finally(() => {
...
function timer() {
  ...
  function setClock(selector, endtime) {
    ...
    const timeInterval = setInterval(updareClock, 1000);
    updareClock();
    function updareClock() {
